...
- **gearmand 2.0.0** (released 2026-07-22, previous release 1.1.22) changes **no wire protocol or packet format**, so `mikespook/gearman-go` stays compatible and needs no adaptation. Its three breaking changes are scheduling behaviour: round-robin job assignment is now the default and can no longer be disabled (good for this worker - it registers 12+ functions on a *single* worker connection, where a high-volume queue like `statusngin_servicechecks` could previously starve latency-sensitive ones like `statusngin_downtimes`), job priority is now global rather than per function, and the client-side task list is FIFO instead of LIFO (that one affects publishers, not this worker).
